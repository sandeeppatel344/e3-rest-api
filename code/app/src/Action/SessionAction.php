...
class SessionAction extends \App\BaseController
{
	...
	private function assignment($data = null)
	{
		if (!is_null($data)) {
			try {
				$stmt = $this->dbConn->select(array('e3erp.session_assignment.name', 'e3erp.user_assignment_status.assignment_status_id', 'e3erp.session_assignment.id as assignment_id', 'e3erp.user_assignment_status.user_comment', 'e3erp.user_assignment_status.approver_comment', '(select Concat(e3erp.person.first_name, " ", e3erp.person.last_name) from e3erp.person where e3erp.person.user_id=e3erp.user_assignment_status.approved_by_user_id) as approved_by'))->from('session_assignment')->join('user_assignment_status', 'user_assignment_status.assignment_id', '=', 'session_assignment.id', 'LEFT OUTER')->join('session', 'session.id', '=', 'session_assignment.session_id')->join('batch', 'batch.id', '=', 'session.batch_id')->join('batch_user', 'batch_user.batch_id', '=', 'batch.id')->where('session_assignment.session_id', '=', $data)->where('batch_user.user_id', '=', $this->userId);
